...
// Set algo for Karbo block template
let mAlgo = 0;
if (cnAlgorithm == "cryptonight" && cnVariant === 0) {
    mAlgo = 0;
} else if (cnAlgorithm == "cryptonight_gpu") {
    mAlgo = 1;
} else if (cnAlgorithm == "cryptonight_power") {
    mAlgo = 2;
}
...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate',
                            {reserve_size: 17, wallet_address: config.poolServer.poolAddress, algo: mAlgo},
                            callback)
}
